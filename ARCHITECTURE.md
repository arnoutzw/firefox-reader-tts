...
Reader TTS is a Firefox Manifest V3 WebExtension paired with a separately operated `travisvn/openai-edge-tts` container.
...
user toolbar action
  ...
  -> cancellable, one-chunk-ahead background fetch to loopback `/v1/audio/speech`
...
- The background validates loopback endpoint, path, chunk length, speed, HTTP result, content type, and non-empty audio. Every request has a 30-second timeout and a request ID used by Stop/replacement/unload cancellation.
...
- Settings live in unencrypted Firefox extension storage. The clear control deletes them. The default token is the non-secret local Compose token `reader-local`.
- Firefox host permissions cover only `localhost` and `127.0.0.1`; arbitrary remote TTS services are intentionally outside this build's trust boundary.
...
`POST http://127.0.0.1:5050/v1/audio/speech` with bearer token and JSON containing `model: tts-1`, one text chunk, `voice: en-US-AvaMultilingualNeural`, `response_format: mp3`, speed, and `stream_format: audio`. Success must be a non-empty `audio/*` body.
